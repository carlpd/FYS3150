...
int main(){
  ...
  PenningTrap part1=PenningTrap(tB0, tV0, td, f, oV, tparts);
  ...
  part2.partint=false;
  std::ofstream r2;
  r2.open("../Txtfiler/r2noint10.txt");
  std::ofstream v2;
  v2.open("../Txtfiler/v2noint10.txt");
  for(int i=0; i<N; i++){
    std::cout<<t(i)<<std::endl;
    ...
    part2.evolve_RK4(dt, t[i]);
  }
  r2.close();
  v2.close();
  PenningTrap part3=PenningTrap(tB0, tV0, td, f, oV, tparts2);
  part3.add_particle(p1);
  part3.add_particle(p2);
  std::ofstream r3;
  std::ofstream v3;
  r3.open("../Txtfiler/r2int10.txt");
  v3.open("../Txtfiler/v2int10.txt");
  for(int i=0; i<N; i++){
    std::cout<<t(i)<<std::endl;
