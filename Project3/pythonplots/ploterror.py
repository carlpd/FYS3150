...
for i in range(dtmax, dtmin):
    t, ax, ay, az, nx, ny, nz, ae, re =np.loadtxt("..//Txtfiler/err{}Euler.txt".format(i), unpack=True)
    plt.plot(t, np.log10(re), label="h=10^-{}".format(i))
    maxEu[i]=np.amax(re)
plt.legend()
plt.savefig("RelerrEu.pdf")
plt.show()
...
for i in range(dtmax, dtmin):
    ...
    maxRK[i]=np.amax(re)
plt.legend()
plt.savefig("relerrRK4.pdf")
plt.show()
print(maxEu)
rerrEu=0
for i in range(dtmin-1, dtmax, -1):
    print(maxEu[i])
    a1=maxEu[i]/maxEu[i-1]
    print(a1)
    a2=10**(-(i+1))/(10**(-i))
    rerrEu+=np.log10(a1)/np.log10(a2)
rerrEU=rerrEu/4
print("rerrEu", rerrEu)
rerrRK=0
for i in range(dtmin-1, dtmax, -1):
